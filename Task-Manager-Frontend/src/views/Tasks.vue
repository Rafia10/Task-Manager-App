...
onMounted(async () => {
  ...
  socket.on('taskUpdated', (updatedData) => {
    if (updatedData._id) {
      const index = TasksData.value.findIndex((item) => item._id === updatedData._id);
      if (index !== -1) {
      ...
      TasksData.value = TasksData.value.filter((item) => item._id !== updatedData._id);
    }
  });
...
<template>
  <div>
    ...
    <div class="mt-8">
      <router-link to="/add-task" class=" bg-blue-500 text-white px-4 py-2 rounded">
        Add Task
      </router-link>
      ...
      <div class="mt-6">
        ...
        <div class="px-4 py-4 -mx-4 overflow-x-auto sm:-mx-8 sm:px-8">
          <div class="inline-block min-w-full overflow-hidden rounded-lg shadow">
            <table class="min-w-full min-w-full divide-y divide-gray-200-normal">
              <thead>
                <tr>
                ...
                <tr v-for="task in TasksData" :key="task?._id">
                  ...
                  <td class="px-5 py-5 text-sm bg-white border-b border-gray-200">
                    <div class="flex flex-col md:flex-row items-center">
                      ...
                      <button @click="handleDelete(task?._id)"
            ...
            <div class="flex flex-col items-center px-5 py-5 bg-white border-t xs:flex-row xs:justify-between">
              <span class="text-xs text-gray-900 xs:text-sm"> {{ task?.totalPages }}</span>

              <div class="inline-flex mt-2 xs:mt-0">
                <button @click="() => { task?.page - 1 }"
